...
public class UsersCommand implements Callable<Integer> {
    ...
    @CommandLine.Option(description = "mongo | mysql", required = true, names = {"-type"})
    private String type;

    @CommandLine.Option(description = "DB Configuration File (xml)", required = true, names = {"-config"})
    ...
    public Integer call() throws Exception {
        ...
        try {
            ServicesFacade.getInstance().init(type, configuration);
            return 0;
        }
        catch (Exception ex) {
            throw  ex;
